...
temp = full(-adj2inc(sparse(triu(A_fault(1:29,1:29))>0)))'; % compute incidence matrix from adjacency matrix
B = [];
for i = 1:size(temp,2) % add columns representing the other direction of the edges
    B(:,(i-1)*2+1) = temp(:,i);
    B(:,(i-1)*2+2) = -temp(:,i);
end
...
x_lower = []; % vector of phase differences with i<j
x_upper = []; % vector of phase differences with i>j
for i = 1:N_load-1
    for j = i+1:N_load
        x_lower(end+1,1) = Phases_evol(end,j)-Phases_evol(end,i)-alpha;
        x_upper(end+1,1) = Phases_evol(end,i)-Phases_evol(end,j)-alpha;
    end
end
...
for i = 1:N_load-1
    for j = i+1:N_load
        c = c+1;
        if A_UT(i,j)==1 % keep only differences between connected oscillators
            xx(end+1,1) = x_lower(c);
            xx(end+1,1) = x_upper(c);
        end
    end
end
