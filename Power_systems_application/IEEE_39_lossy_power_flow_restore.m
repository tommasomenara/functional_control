...
% method on the IEEE 39 New England test case during a simulated fault in a
% power network with lossy connections
...
Phases_evol = Kuramoto_fun_lossy(A(1:29, 1:29), 1, N_load, alpha, time, w, theta_0(1:29));
...
Phases_evol_after_fault = Kuramoto_fun_lossy(A_fault(1:29, 1:29), 1, N_load, alpha, time, w, Phases_evol(end,:)');
...
if flag == 1
    Phases_evol_fixed = Kuramoto_fun_lossy(Solution, 1, N_load, alpha, time, w, Phases_evol_after_fault(end,:));
...
%% compute max and mean error between original (pre-fault) and recovered functional patterns

max_error = max(abs(rho_matrix(:)-rho_matrix_fixed(:)))
mean_error = mean(abs(rho_matrix(:)-rho_matrix_fixed(:)))